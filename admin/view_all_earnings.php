...
//$getAllOrders = $UF->getAllOrders($created_min,$created_max,$store,$orderNumber,$paymentType);
$getAllOrders = $UF->getOrdersFromAllStores($created_min,$created_max,$store,$orderNumber,$paymentType);

/*------code with getOrderEarnings----------*/

    //$earningOrders = json_decode($allOrdersData['response'],true);
    //$ordersStatus = array();
    //
    //foreach($earningOrders as $order){
    //    $ordersStatus[] = $order;
    //}

/*------end of code with getOrderEarnings----------*/

/*------code with getOrdersFromAllStores-----------*/

$ordersStatus = array();
for($i=0; $i<count($getAllOrders); $i++){
    $getAllOrdersJson = json_decode($getAllOrders[$i],true);
    foreach($getAllOrdersJson as $order){
        $ordersStatus[] = $order;
    }
}
/*-------end of code with getOrdersFromAllStores----*/
                        ...
                        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                            ...
                            <div class="my-products__filter">
                                ...
                                <div class="input-field my-products__filter-item col-sm-3">
                                    <input id="orderNumber" name="orderNumber" type="text" placeholder="Order Number" class="form-control">
                                </div>
                                <div class="input-field my-products__filter-item my-products__filter-item--keywords col-sm-3">
...
//                                    print(floatval($cost - $cutAmount));
                                    /*----------saveCode--------------*/
                                    $sellingPrice = $item['total_price'] + $profit;
                                    $markup = ($sellingPrice - $item['total_price']);
                                    //$markupPercentage = (($sellingPrice - $item['total_price']) / $item['total_price']);  //for markup percentage
                                    echo $markup;
